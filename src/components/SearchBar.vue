<template>
	<popup v-bind:isOpen.sync="isFocused">
		<div
			class="search-bar"
			v-bind:class="{ 'search-bar-focused': isFocused }"
		>
			<input
				autocomplete="off"
				spellcheck="false"
			...
			<label
				v-on:click="search"
				class="search-bar-btn iconfont icon-arrowright"
			></label>
		</div>
		...
		<div
			...
			v-bind:class="{
				'search-bar-filter-open': isFilterOpen && isFocused
			}"
		>
			<div>
				Mode:
